...
class CustomModel(keras.Model):
    def test_step(self, data):
        ...
        # signed_grad = backend.sign(gradients)
        # adv = x + signed_grad * eps
...
# origin_train = pd.read_csv('combined_iot23_normalize.csv', header=None, sep=',', comment='#', low_memory=False)
# origin_test = pd.read_csv('combined_iot23_label.csv', names=['label'], comment='#', low_memory=False)

# (X_train, X_test, Y_train, Y_test) = train_test_split(origin_train, origin_test, test_size=0.2, random_state=0)
...
#     print("Combined normal and adv data")
...
#     # second_train_data.to_csv(f'./{status}/{eps}/second_train_data.csv', index=False, header=False)
#     # second_train_label.to_csv(f'./{status}/{eps}/second_train_label.csv',index=False, header=False)
...
#     # second_test_data.to_csv(f'./{status}/{eps}/second_test_data.csv', index=False, header=False)
#     # second_test_label.to_csv(f'./{status}/{eps}/second_test_label.csv',index=False, header=False)

#     print("Training adv model...")
