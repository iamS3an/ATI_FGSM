...
for eps in [0.05, 0.1, 0.01, 0.02]:
    ...
    train_adv_label = pd.read_csv(f'./{status}/{eps}/train_adv_label.csv', header=None, sep=',', comment='#', low_memory=False)
    test_adv_label = pd.read_csv(f'./{status}/{eps}/test_adv_label.csv', header=None, sep=',', comment='#', low_memory=False)
    ...
    second_train_label = pd.concat([Y_train, Y_train])
    ...
    second_test_label = pd.concat([Y_test, Y_test])
